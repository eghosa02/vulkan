...
namespace lve {
    EngineSwapChain::EngineSwapChain(EngineDevice& deviceRef, VkExtent2D extent) : device{ deviceRef }, windowExtent{ extent }
    {
        init();
    }
    EngineSwapChain::EngineSwapChain(EngineDevice& deviceRef, VkExtent2D extent, std::shared_ptr<EngineSwapChain> prev): device{ deviceRef }, windowExtent{ extent }, oldSwapChain(prev)
    {
        init();
    ...
    VkResult EngineSwapChain::submitCommandBuffers(const VkCommandBuffer* buffers, uint32_t* imageIndex) {
        ...
        if (vkResetFences(device.device(), 1, &inFlightFences[currentFrame]) != VK_SUCCESS)
    ...
    bool EngineSwapChain::compareSwapFormats(const EngineSwapChain& swapChain) const
    {
        return swapChain.swapChainFormat == swapChainFormat && swapChain.swapChainImageFormat == swapChainImageFormat;
    }
